                        ...
                        <div id="accordion">
                            ...
                            <div class="tabs-to-dropdown">
                                    ...
                                    <ul class="nav nav-pills d-md-flex" id="pills-tab" role="tablist">
                                        @foreach ($appointment->training->media as $key => $media)
                                            <li class="nav-item" role="presentation">
                                                <a class="nav-link {{ $key == 0 ? 'active bg-success text-white' : '' }} tabItem fileItem @if($key > 0)unseen @endif"
                                        ...
                                        @if(!empty($appointment->training->resources))
                                            <li class="nav-item" role="presentation">
                                                <a class="nav-link tabItem" data-toggle="pill" href="#resources"
                                                   role="tab" aria-controls="pills-company" aria-selected="true">რესურსები</a>
                                            </li>
                                        @endif
                                    </ul>
                                </div>
                                ...
                                @if(count($appointment->training->media) > 0)
                                    <h5 class="file-title"
                                        id="file-title">{{ $appointment->training->media[0]->name }}</h5>
                                @endif

                                <div class="tab-content" id="pills-tabContent">
                                    @foreach ($appointment->training->media as $key => $media)
                                        <div class="tab-pane fade show {{ $key == 0 ? 'active' : '' }} "
                                             id="tab_{{ $media->id }}" role="tabpanel"
                                             aria-labelledby="tab_{{ $media->id }}">
                                            <div class="">
                                                <div class="">
                                                    @if($media->type == \App\Models\TrainingMedia::TYPE_DOCUMENT)
                                                        <embed src="{{ amazon_s3_url($media->path) }}#toolbar=0"
                                                               type="application/pdf" width="100%" height="600px"/>
                                                    @elseif($media->type == \App\Models\TrainingMedia::TYPE_VIDEO)
                                                        <video width="320" height="240" controls
                                                               controlsList="nodownload">
                                                            <source src="{{ amazon_s3_url($media->path) }}"
                                                                    type="video/mp4">
                                        ...
                                        <div class="tab-pane fade" id="resources" role="tabpanel"
                                            ...
                                            <div class="container-fluid" style="height: 609px; background: #fff">
                                                ...
                                                @foreach (json_decode($appointment->training->resources) as $ka => $resource)
                                                    ...
                                                    text-decoration-color: #a2dffb; color:#0089c7" target="_blank"
                    ...
                    <div class="col-md-12 col-lg-12 mt-5">
                        <div style="display: inline-block; float:right">
                            @if(auth()->guard(\App\Services\AuthType::TYPE_EXTERNAL_CUSTOMER)->check())
                                <a target="_blank" href="{{ route('front.startExternalTestView', $appointment) }}"
