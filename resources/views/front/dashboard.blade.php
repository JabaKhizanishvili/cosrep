                        ...
                        <div class="themeioan_event">
                            <article><!-- single event start -->
                                @if($appointment->isFinishedByCustomer())
                                    <div class="event-photo {{ $appointment->pivot->final_point >= $appointment->pivot->point_to_pass ? 'testStatusSuccess' : 'testStatusFail' }}">
                                @else
                                    <div class="event-photo {{ $appointment->isDone() ? 'testNeverFinished' : '' }}">
                                @endif
                                    <div class="date">
                                        <h4><span>{{ date('d', strtotime($appointment->start_date)) }}</span> {{ montName(date('m', strtotime($appointment->start_date))) }}, {{ date('Y', strtotime($appointment->start_date)) }}</h4>
                                    </div>
                                    <img src="{{ trainingImageThumb($appointment->training->image) }}" alt="">
                                </div>
                                <div class="event-content">
                                    ...
                                    <div class="course-viewer">
                                        <ul>
                                            <li><i class="fas fa-clock"></i> {{ date('H:i', strtotime($appointment->start_date)) }} - {{ date('H:i', strtotime($appointment->end_date)) }}</li>
                                        </ul>
                                    ...
                                    @if(empty($appointment->pivot->finished_at))
                                        <div class="btn-section">
                                            ...
                                            <a href="{{ route('front.startTrainingView', $appointment) }}" class="btn btn-success text-white pt-1 pb-1 startTrainingBtn">ტრენინგის დაწყება</a>
                                            @else
                                            <br>
                                            @endif
                                        </div>
                                    @else
                                        ...
                                        <div class="btn-section">
                                            <a href="{{ route('front.testDetails', $appointment) }}" class="button-light"><i class="fas fa-arrow-right"></i>ტესტის დეტალები</a>
                                        </div>
                        ...
                        <div class="themeioan_services">
                            <article><!-- single services -->
                                <div class="blog-content">
                                    ...
                                    <h5 class="title"><a href="services-detail.html" class="float-left">{{ $customer->name }}</a>
                                    <br>
                                    </h5>
                                    <p>ელ ფოსტა: {{ $customer->email }}</p>
                                    <p>რეგისტრაციის თარიღი: {{ date("Y-m-d H:i", strtotime($customer->created_at)) }}</p>
